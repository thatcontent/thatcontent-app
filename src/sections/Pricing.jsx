  ...
  return (
    <section id="packages" className="relative py-24 px-6">
      <div className="max-w-7xl mx-auto">
          ...
          {packages.map((pkg, i) => (
            <div 
              ...
              className={`glass-panel p-8 relative flex flex-col h-full ${
                pkg.highlight 
                  ? 'border-[#F97316]/50 bg-white/10 shadow-[0_0_40px_rgba(249,115,22,0.15)] transform lg:-translate-y-4' 
                  : 'opacity-90 hover:opacity-100'
              }`}
            >
              {pkg.badge && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-[#EA580C] to-[#F97316] text-black text-xs font-bold px-4 py-1.5 rounded-full tracking-wider">
                  {pkg.badge}
                </div>
              )}
              
              <div className="border-b border-white/10 pb-8 mb-8 mt-4">
              ...
              <ul className="space-y-4 mb-10 flex-grow">
                {pkg.features.map((feat, index) => (
                  <li key={index} className="flex items-start gap-3 text-sm text-white/80">
                    <Check size={16} className={`shrink-0 mt-0.5 ${pkg.highlight ? 'text-[#F97316]' : 'text-white/40'}`} />
                    <span>{feat}</span>
                  </li>
                ))}
              </ul>
              ...
              <button className={`w-full py-3 rounded-full text-sm font-medium transition-all duration-300 ${
                pkg.highlight 
                  ? 'bg-gradient-to-r from-[#EA580C] to-[#F97316] text-black hover:shadow-[0_0_20px_rgba(249,115,22,0.4)]' 
        ...
        {/* IT Pricing Note */}
        <div className="mt-24 glass-panel p-10 text-center max-w-4xl mx-auto bg-gradient-to-b from-white/5 to-transparent">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <button className="btn-primary">Discuss My Digital Project</button>
            <button className="px-6 py-2.5 rounded-full bg-transparent border border-white/10 text-white text-sm font-medium hover:bg-white/5">
